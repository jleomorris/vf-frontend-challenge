...
locals {
  terragrunt_state_prefix = try(get_env("TERRAGRUNT_STATE_PREFIX"), "cicd")
  ...
  environment_tfvars = jsondecode(file("${get_path_to_repo_root()}iac/terraform_aws/environments/${local.terragrunt_identifier}.${local.terragrunt_environment}.${local.terragrunt_aws_region}.tfvars.json"))
}
...
terraform {
  ...
  extra_arguments "common_var" {
    ...
    arguments = ["-var-file=../../../../../environments/${local.terragrunt_identifier}.${local.terragrunt_environment}.${local.terragrunt_aws_region}.tfvars.json"]
  }
